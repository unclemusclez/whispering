  ...
  return (
    <Card className="w-full max-w-xl">
      ...
      <CardContent className="space-y-6">
        ...
        {settings["transcription.selectedTranscriptionService"] === "Groq" && (
          <div className="grid gap-2">
            <LabeledInput
              id="groq-api-key"
              label="Groq API Key"
        ...
        {settings["transcription.selectedTranscriptionService"] ===
          ...
          <>
            <div className="grid gap-2">
              <LabeledInput
                id="whispercpp-server-url"
                label="Whisper.cpp Server URL"
                value={settings["transcription.whisperCpp.serverUrl"]}
                onChange={(value) =>
                  setSettings({
                    ...settings,
                    "transcription.whisperCpp.serverUrl": value,
                  })
                }
              ...
              />
              <div className="text-muted-foreground text-sm">
                The URL of your running whisper.cpp server (e.g.,
                http://127.0.0.1:8080).
              </div>
            </div>
            <div className="grid gap-2">
              <LabeledInput
                ...
                value={settings["transcription.whisperCpp.model"]}
                onChange={(value) =>
                  setSettings({
                    ...settings,
                    "transcription.whisperCpp.model": value,
                  })
                }
                placeholder="whisper/ggml-large-v3-turbo-q8_0.bin"
                type="text"
              />
              <div className="text-muted-foreground text-sm">
                Path to the model file on the server (e.g.,
            ...
            <div className="flex items-center gap-2">
              <Switch
                id="ssl-verify"
                aria-labelledby="ssl-verify"
                checked={settings["transcription.sslVerify"]}
                onCheckedChange={(newValue) =>
                  setSettings({
                    ...settings,
                    "transcription.sslVerify": newValue,
                  })
                }
              />
              <Label htmlFor="ssl-verify">
                Verify SSL (disable for local testing only)
              </Label>
            </div>
          </>
        )}
        ...
        <Button
          onClick={() => {
            ...
            if (
              settings["transcription.selectedTranscriptionService"] ===
                "WhisperCpp" &&
              (!settings["transcription.whisperCpp.serverUrl"] ||
                !settings["transcription.whisperCpp.model"])
            ) {
              alert(
                "Please enter both a Whisper.cpp server URL and model path"
              );
              return;
            }
            window.close();
